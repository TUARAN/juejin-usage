...
import { createJsonlLineReader, readJsonlTail } from '../src/parsers/jsonl-tail.js';
...
test('createJsonlLineReader exposes the consumed offset after iteration', async () => {
  const complete = '{"a":1}\n';
  const path = await tempFile('i.jsonl', `${complete}{"a":`);
  const reader = createJsonlLineReader(path);
  const lines: string[] = [];
  for await (const line of reader) lines.push(line);
  assert.deepEqual(lines, ['{"a":1}']);
  assert.equal(reader.nextOffset, Buffer.byteLength(complete));
  assert.equal(reader.pendingPartial, true);
});
