...
function sampleBucket(overrides: Partial<QueueBucket> = {}): QueueBucket {
  return {
...
test('local metrics use disjoint tokens and weighted cache ratios', () => {
  const metrics = metricsFromBucket(sampleBucket());
...
test('missing, invalid, legacy and zero usage remain distinct', () => {
  const legacy = sampleBucket({ local_metrics: undefined });
  assert.equal(metricsFromBucket(legacy).requestCount, null);
  assert.equal(metricsFromBucket(legacy).knownRequestCount, 1);
  assert.equal(metricsFromBucket(legacy).cacheHitRate, 0.6);
  const mixed = aggregateLocalMetrics([legacy, sampleBucket()]);
  assert.equal(mixed.requestCount, null);
  assert.equal(mixed.knownRequestCount, 2);
  assert.equal(
    metricsFromBucket(
      sampleBucket({ source: 'unknown', local_metrics: undefined }),
    ).cacheHitRate,
    null,
  );
  assert.equal(
  ...
  assert.equal(
    metricsFromBucket(sampleBucket({ input_tokens: Infinity }))
      .uncachedInputTokens,
    0,
  );
  const zero = sampleBucket({
    input_tokens: 0,
  ...
  assert.equal(
    metricsFromBucket(sampleBucket({ cached_input_tokens: 0 })).cacheHitRate,
    0,
  );
  assert.equal(
...
test('unknown model alignment conserves request evidence and upload hashes ignore it', () => {
  const rows = [sampleBucket(), sampleBucket({ model: 'unknown' })];
...
test('cached and uncached local API contracts agree after reload and metrics-only reseal', async (t) => {
  const dir = await mkdtemp(join(tmpdir(), 'local-api-metrics-'));
  t.after(() => rm(dir, { recursive: true, force: true }));
  const yesterday = addLocalDays(localDateNow(), -1);
  const rows = [
    sampleBucket(),
    sampleBucket({ hour_start: `${yesterday}T02:00:00.000Z` }),
    sampleBucket({
